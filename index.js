...
do 
var answer_2 = Math.floor(Math.random()*10)
while (answer_2 === answer_1);
answer.push(answer_2)

do 
var answer_3 = Math.floor(Math.random()*10)
...
function find_out_correct(replay_1,replay_2,replay_3) {
  ...
  let maru_count = symbol.filter(function(x){return x==='○'}).length;
  let sannkaku_count = symbol.filter(function(x){return x==='△'}).length;
  let batsu_count = symbol.filter(function(x){return x==='X'}).length;
